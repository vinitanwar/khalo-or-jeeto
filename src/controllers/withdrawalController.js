...
const createWithdrawalRequest = async (req, res) => {
  ...
  try {
    ...
    if (withdrawalMethod === WITHDRAWAL_METHODS_MAP.BANK_CARD) {
      ...
      if (user.level == 3) {
        var msg = "Withdrawal Successfully!";
      ...
      var msg = "Withdrawal request registered Successfully!";
      withdrawDB.createBankCardWithdrawalRequest({
        userPhoneNumber: user.phone,
        bankName: account.bankName,
        recipientName: account.recipientName,
...
const getUserDataByAuthToken = async (authToken) => {
  let [users] = await connection.query(
    "SELECT `phone`, `code`,`name_user`,`invite`,`money`,`level` FROM users WHERE `token` = ? ",
    [authToken],
  );
  const user = users?.[0];
...
const withdrawDB = {
  ...
  async createUSDTWithdrawalRequest({
    ...
    await connection.query(
      `INSERT INTO withdraw SET id_order = '${getOrderId()}', phone = '${userPhoneNumber}', name_bank = '${mainNetwork}', stk = '${usdtAddress}', sdt = '${addressAlias}', tp = '${type}', time = '${time}', today = '${getTodayString()}', money = '${amount}', status = '${status}'`,
    );
  },
  async createBankCardWithdrawalRequest({
    ...
    const type = WITHDRAWAL_METHODS_MAP.BANK_CARD;
    console.log("userPhoneNumber", status);
    await connection.query(
      `INSERT INTO withdraw SET id_order = '${getOrderId()}', phone = '${userPhoneNumber}', name_bank = '${bankName}', name_user = '${recipientName}', stk = '${bankAccountNumber}', ifsc = '${IFSC}', sdt = '${upiId}', tp = '${type}', time = '${time}', today = '${getTodayString()}', money = '${amount}', status = '${status}'`,
    );
  },
