...
const initiateRspayPayment = async (req, res) => {
  ...
  let amount = parseFloat(req.body.am);
...
const initiateWepayPayment = async (req, res) => {
  const type = PaymentMethodsMap.WEPAY;
  let auth = req.cookies.auth;
  ...
  try {
    ...
    let merchantId = '711000011';
    let merchantKey = '434584CDDC940318FBE2BB7F1E90579C';
    ...
    let params = {
      notify_url: `${process.env.APP_BASE_URL}/notice`,
      callback_url: `${ process.env.APP_BASE_URL }/wallet/rechargerecord`,
      merchantid: "711000011",
      passage_code: '100001',
      merchant_orderno: "123124141",
      account: "6123199999",
      currency: "INR",
      amount: 100,
      // otherData: "{'user':'1'}",
    ...
    params["sign"] = wepay.generateSign(params, merchantKey);
    console.log('params', params);
    const response = await axios({
      method: "POST",
      url: "http://8.219.86.200/mcapi/prepaidorder/v2",
      data: params,
      headers: {
...
const wepay = {
  generateSign: (params, privateKey) => {
    // Step 1: Sort keys in ASCII order
    const sortedKeys = Object.keys(params)
      .filter(key => key !== "sign" && params[key] !== null && params[key] !== "") // Exclude 'sign' and empty/null values
      .sort();

    // Step 2: Create key=value pairs and concatenate
    let stringSignTemp = sortedKeys
      .map(key => `${key}=${params[key].toString().trim()}`) // Ensure values are trimmed
      .join("&");

    // Step 3: Append the private key
    stringSignTemp += `&key=${privateKey}`;

    console.log("待签名值:", stringSignTemp); // Log the string before hashing

    // Step 4: Generate MD5 hash and convert to lowercase
