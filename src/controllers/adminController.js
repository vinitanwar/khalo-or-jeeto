...
const demoPage = async (req, res) => {
  return res.render("manage/demo.ejs");
};
...
const demostatistical = async (req, res) => {
  return res.render("manage/demostatistical.ejs");
};
...
const listMember = async (req, res) => {
  try {
    ...
    // Destructure and validate pagination inputs
    ...
    const [users] = await connection.execute(sql, [limit, offset]);
    console.log("users", users);

    // Execute the count query (no params needed since no placeholders are used)
    const [totalUsersResult] = await connection.execute(countSql);
...
const listDEMO = async (req, res) => {
  ...
  const [wingo] = await connection.query(
    `SELECT * FROM users WHERE veri = 1 AND level = 3 ORDER BY id DESC LIMIT ${pageno}, ${pageto} `,
  );
  return res.status(200).json({
    message: "Success",
    status: true,
...
const demostatistical2 = async (req, res) => {
  const [wingo] = await connection.query(
    `SELECT SUM(money) as total FROM minutes_1 WHERE status = 1 `,
  );
  const [wingo2] = await connection.query(
  ...
  const [users] = await connection.query(
    `SELECT COUNT(id) as total FROM users WHERE status = 1 and level = 3 `,
  );
  const [users2] = await connection.query(
    `SELECT COUNT(id) as total FROM users WHERE status = 0 and level = 3 `,
  );
  const [recharge] = await connection.query(
    `SELECT SUM(money) as total FROM recharge WHERE status = 1 `,
  );
...
const lockwithdraw = async (req, res) => {
  let auth = req.cookies.auth;
  let id = req.body.id;
  let type = req.body.type;
  if (!auth || !id || !type) {
  ...
  if (type == "lock") {
    await connection.query(`UPDATE users SET withdraw = 1 WHERE id = ?`, [id]);
    return res.status(200).json({
  ...
  if (type == "unlock") {
    await connection.query(`UPDATE users SET withdraw = 0 WHERE id = ?`, [id]);
    return res.status(200).json({
      message: "Withdrawal unlocked successfully",
      status: true,
    });
  }
};
...
const registerdemo = async (req, res) => {
  ...
  try {
    ...
    if (check_u.length == 1) {
      ...
      const sql =
        "INSERT INTO users SET id_user = ?,phone = ?,name_user = ?,password = ?,plain_password = ?, money = ?,bonus_money = ?,code = ?,invite = ?,veri = ?,ip_address = ?,status = ?,time = ?,level = 3";
      await connection.execute(sql, [
        id_user,
        username,
        name_user,
      ...
      return res.status(200).json({
        message: "demo registration success", //Register Sucess
        status: true,
      });
    }
  } catch (error) {
...
const CreatedSalary = async (req, res) => {
  try {
    ...
    if (!phone) {
      return res.status(400).json({
        message: "ERROR!!! Phone or ID user must be provided.",
        status: false,
      });
    }

    const userQuery = "SELECT * FROM `users` WHERE phone = ? OR id_user = ?";
    const [existingUser] = await connection.execute(userQuery, [phone, phone]);

    if (existingUser.length === 0) {
      return res.status(400).json({
        message: "ERROR!!! User with the provided phone or ID user does not exist.",
        status: false,
      });
    }

    const userPhone = existingUser[0].phone;

    const updateUserQuery =
      "UPDATE `users` SET `money` = `money` + ? WHERE phone = ?";
    await connection.execute(updateUserQuery, [amount, userPhone]);

    const insertSalaryQuery =
      "INSERT INTO salary (phone, amount, type, time, uid) VALUES (?, ?, ?, ?, ?)";
    await connection.execute(insertSalaryQuery, [
      userPhone,
      amount,
      type,
      now,
      existingUser[0].id_user,
    ]);
